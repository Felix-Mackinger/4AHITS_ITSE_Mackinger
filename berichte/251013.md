...
3. Recherchiere zu den Optionen von ping, erstelle ein kleines Cheat Sheet
    ...
    -c <count> — sende genau N Echo Requests (z.B. -c 4).
    -W <timeout> — Wartezeit (in Sekunden) für jede Antwort (response timeout).
    -w <deadline> — Gesamt-Deadline in Sekunden für den gesamten ping-Befehl.
    -i <interval> — Intervall zwischen Paketen (Standard 1s).
    -s <size> — Payload-Größe in Bytes (z. B. -s 1000).
    -t <ttl> — Setze TTL (Time To Live).
    -q — quiet, nur Statistik ausgeben.
    -n — keine Namensauflösung / nur numerische Adressen (schneller).
    -4 / -6 — nur IPv4 / nur IPv6.
    -I <interface|source> — Interface oder Source-IP für ausgehende Pakete.
    -D — Timestamp (Unix epoch) vor jeder Zeile (hilfreich fürs Logging).

4. Welchen Netzwerkdienst verwendet der ping Dienst? Recherchiere wie Firewalls häufig mit ping umgehen.

    **ICMP** *I*nternet *C*ontrol *M*essage *C*ontrol benutzt Echo Request - Echo Reply. Das ICMP-Protokoll ist Teil der IP-Familie und ist kein TCP/UDP-Dienst


    - **Blocken**: Viele externe Firewalls blocken eingehende ICMP Echo Requests aus dem Internet (Sicherheits-/DoS-Gründe).
    - **Rate-Limit**: ICMP wird oft gedrosselt — einzelne Antworten kommen, Dauer-Scans werden limitiert.
    - **Allow internal / block external**: In LANs sind ICMPs häufig erlaubt (Troubleshooting), aber aus dem WAN oft nicht.
    - **Selective Allow**: Manche Firewalls erlauben nur destination-unreachable/ttl exceeded und blockieren Echo.
    - **IDS/IPS**: Bei verdächtigem Scan-Verhalten kann IDS/IPS die Quelle temporär blockieren.
...
```sh
┌──(kali㉿kali)-[~]
└─$ ping -c 6 192.168.12.254
PING 192.168.12.254 (192.168.12.254) 56(84) bytes of data.
64 bytes from 192.168.12.254: icmp_seq=1 ttl=255 time=0.559 ms
64 bytes from 192.168.12.254: icmp_seq=2 ttl=255 time=1.60 ms
64 bytes from 192.168.12.254: icmp_seq=3 ttl=255 time=1.60 ms
64 bytes from 192.168.12.254: icmp_seq=4 ttl=255 time=0.768 ms
64 bytes from 192.168.12.254: icmp_seq=5 ttl=255 time=0.769 ms
64 bytes from 192.168.12.254: icmp_seq=6 ttl=255 time=1.49 ms

--- 192.168.12.254 ping statistics ---
6 packets transmitted, 6 received, 0% packet loss, time 5234ms
rtt min/avg/max/mdev = 0.559/1.129/1.597/0.438 ms
...
3. nmap benötigt als Argument den IP Adressbereich des Subnet in Präfix Notation (z.B.: 192.168.178.0/24)


```sh
┌──(kali㉿kali)-[~]
└─$ nmap -sS -sV -p22,80,443 192.168.12.52
Starting Nmap 7.95 ( https://nmap.org ) at 2025-10-20 06:46 EDT
Nmap scan report for 192.168.12.52
...
4. Mit der Option -sn führt nmap nur den ping scan durch (ansonsten wird automtisch auch die nächste, länger dauernde, Phase des scans ausgeführt – siehe folgende Übung)
